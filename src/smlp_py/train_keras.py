...
class ModelKeras:
    ...
    def model_checkpoint_pattern(self):
        ...
        return (
            self.model_file_prefix + "_model_checkpoint.keras"
        )  #'_model_checkpoint.h5'
    ...
    def _param_dict_with_algo_name(self, param_dict, algo):
        ...
        for k, v in param_dict.items():
            v_updated = v.copy()
            v_updated["abbr"] = self._hparam_name_local_to_global(v["abbr"], algo)
            result_dict[self._hparam_name_local_to_global(k, algo)] = v_updated
        return result_dict
    ...
    def _report_training_regression(
        ...
        for metric in metrics:
            ...
            if len(resp_names) == 1 or history_for_all_responses:
                acc = history.history[metric.name]
                val_acc = history.history["val_" + metric.name]
                if len(resp_names) == 1:
