...
def nn_init_model(input_dim, optimizer, activation, layers_spec, n_out):
    ...
    for fraction in map(float, layers_spec.split(",")):
        ...
        model.add(
            keras.layers.Dense(
                n, activation=activation, input_dim=input_dim if first else None
            )
        )
        first = False
...
def nn_init_model_functional(input_dim, optimizer, activation, layers_spec, resp_names):
    ...
    for i in range(len(layers_spec_list)):
        ...
        if i == 0:
            ...
            globals()[nn_layer_names[i]] = keras.layers.Dense(
                n, activation=activation, input_dim=None
            )(globals()[nn_layer_names[i - 1]])
    ...
    for resp in resp_names:
        globals()[f"{resp}"] = keras.layers.Dense(
            1, name=resp, activation=OUT_ACTIVATION
        )(globals()[nn_layer_names[-1]])
...
def nn_train(
    model, epochs, batch_size, model_checkpoint_path, X_train, X_test, y_train, y_test
):
    checkpointer = None
    ...
    if SEQUENTIAL_MODEL:
        history = model.fit(
            ...
            #                   steps_per_epoch=10,
            callbacks=[
                c for c in (checkpointer, earlyStopping, rlrop) if c is not None
            ],
...
def plot_true_pred_runtime0(y, y_pred, title, out_prefix=None, log_scale=False):
    try:
        plot_true_pred_runtime1(
            y, y_pred, title, out_prefix=out_prefix, log_scale=log_scale
        )
...
def plot_true_pred_runtime(
    y, y_pred, title, out_prefix=None, log_scale=False, objective=None
):
    plot_true_pred_runtime0(
        y, y_pred, title, out_prefix=out_prefix, log_scale=log_scale
    )
    if objective is not None:
        plot_true_pred_runtime0(
            DataFrame(y.eval(objective), columns=[objective]),
            DataFrame(
                DataFrame(y_pred, columns=y.columns).eval(objective),
                columns=[objective],
            ),
            title + "-obj",
...
def nn_main(
    ...
    persist = {
        ...
        )(MinMaxScaler().fit(DataFrame(data.eval(objective))))
        if objective is not None
        else None,
        "train": {
            "epochs": epochs,
    ...
    if SEQUENTIAL_MODEL:
        model = nn_init_model(
            input_dim, optimizer, activation, layers_spec, len(resp_names)
        )
    else:
        model = nn_init_model_functional(
            input_dim, optimizer, activation, layers_spec, resp_names
        )
    history = nn_train(
        model, epochs, batch_size, chkpt_pattern, X_train, X_test, y_train, y_test
    )
