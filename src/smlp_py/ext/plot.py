...
class plot_exp:
    def __init__(self, exp=exp, setno="15"):
        ...
        self.stable_x_original_html_path = (
            f"Set{self.setno}_{self.exp}_stable_x_original.html"
        )
        self.stable_x_counter_html_path = (
            f"Set{self.setno}_{self.exp}_stable_x_counter.html"
        )
        ...
        if isinstance(data, list) and key == "arguments":
            ...
            while i < len(data):
                ...
                if item.startswith("-"):
                    ...
                    # Store in dictionary
                    arguments_dict[k] = self._convert_value(
                        v
    ...
    def plott(self, x, y, data_version):
        ...
        if len(orig_data.columns) > 2:
            if data_version == "optimized":
                ...
                # Save figure as HTML
                fig1.write_html(
                    f"Set{self.setno}_{self.exp}_scatter_plot_optimized.html"
                )
                ...
                # Save figure as HTML
                fig0.write_html(
                    f"Set{self.setno}_{self.exp}_scatter_plot_predicted.html"
                )
            ...
            plt.title(
                "Scatter Plot representing Original data and Model Reconstruction of Original Data/Graph"
            )
            plt.grid(True)
            plt.legend()
    ...
    def witnesses(self):
        ...
        if os.path.exists(self.witnesses_json):
            ...
            if x_witnesses is None:
                ...
                logging.warning(
                    f"Ignoring invalid 'witnesses/x' type: {type(x_witnesses)}"
                )
            ...
            if set_dim == 3:
                ...
                if "counter" in data and "stable" in data:
                    ...
                    fig2 = go.Figure(
                        data=[
                            go.Scatter3d(
                                ...
                                mode="markers",
                                marker=dict(
                                    color=z_orig, colorscale="Viridis", opacity=0.5
                                ),
                                name="Original data",
                            ),
                            go.Scatter3d(
                    ...
                    if len(z) <= 10:
                        ...
                        z_np = np.array(
                            z, dtype=float
                        )  # Convert to a NumPy array (use float for division)

                        min_z = np.min(z_np)  # Use NumPy's min function
                        max_z = np.max(z_np)  # Use NumPy's max function

                        if max_z == min_z:
                            z_normalized_np = np.full(
                                z_np.shape, 0.5
                            )  # Create an array of 0.5s
                        else:
                        ...
                        fig2.data[1].marker.update(
                            ...
                            colorbar=dict(
                                ...
                                tickvals=np.linspace(
                                    0, 1, 11
                                ),  # 11 ticks for 10 intervals
                                ticktext=[
                                    f"{min(z) + i * (max(z) - min(z)) / 10:.2f}"
                                    for i in range(11)
                                ],
                            ),
                        )
                        ...
                        fig2 = go.Figure(
                            data=[
                                ...
                                go.Scatter3d(
                                    ...
                                    mode="markers",
                                    marker=dict(
                                        color=z_orig, opacity=0.5, colorscale="Viridis"
                                    ),
                                    name="original data",
                                ),
                            ]
