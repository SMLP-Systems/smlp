...
class SmlpDoepy:
    def __init__(self):
        ...
        self.TWO_LEVEL_DESIGNS = [
            self.TWO_LEVEL_FRACTIONAL_FACTORIAL,
            self.PLACKET_BURMAN,
            self.SUKHAREV_GRID,
            self.BOX_WILSON,
            self.LATIN_HYPERCUBE,
            self.LATIN_HYPERCUBE_PROB_DISTR,
            self.RANDOM_K_MEANS,
            self.MAXMIN_RECONSTRUCTION,
        ...
        self.doepy_params_dict = {
            "doe_algo": {
                ...
                + 'The supported algorithms are: "{}, {}, {}, {}, {}, {}, {}, {}, {}, {}"'.format(
                    self.FULL_FACTORIAL,
                    self.TWO_LEVEL_FRACTIONAL_FACTORIAL,
                    self.PLACKET_BURMAN,
                    self.SUKHAREV_GRID,
                    ...
                    self.BOX_WILSON,
                    self.LATIN_HYPERCUBE,
                    self.LATIN_HYPERCUBE_SPACE_FILLING,
                    self.HALTON_SEQUENCE,
                    self.UNIFORM_RANDOM_MATRIX,
                ),
            },
            "doe_factor_level_ranges": {
                "abbr": "doe_factor_level_ranges",
                "type": dict,
            ...
            "doe_num_samples": {
                "abbr": "doe_samples",
                "type": int,
                "help": "Number of samples (experiments) to be generated [default: {}]".format(
                    str(None)
                ),
            },
            "doe_design_resolution": {
                "abbr": "doe_resolution",
                "default": None,
                "type": int,
            ...
            "doe_box_behnken_centers": {
                "abbr": "doe_bb_centers",
                "default": self.BOX_BEHNKEN_CENTERS,
                "type": int,
                "help": "Number of center points to include in the final design "
                + "[default: {}]".format(str(self.BOX_BEHNKEN_CENTERS)),
            },
            "doe_central_composite_center": {
                "abbr": "doe_cc_center",
                "default": self.BOX_WILSON_CENTER,
                "type": str,
                "help": "A 1-by-2 array of integers, the number of center points in each block of the design. "
                + "[default]",
            },
            "doe_central_composite_alpha": {
                "abbr": "doe_cc_alpha",
                "default": self.BOX_WILSON_ALPHA,
                "type": str,
                "help": 'A string describing the effect of alpha has on the variance. "alpha" can take on two '
                + 'values: "orthogonal" or "o", and "rotatable" or "r" [default {}]'.format(
                    str(self.BOX_WILSON_ALPHA)
                ),
            },
            "doe_central_composite_face": {
                "abbr": "doe_cc_face",
                "default": self.BOX_WILSON_FACE,
                "type": str,
                ...
                + "      space, so "
                'alpha" = 1. This variety requires 3 levels of each factor. Augmenting an existing '
            ...
            "doe_prob_distribution": {
                "abbr": "doe_prob_distr",
                "default": self.LATIN_HYPERCUBE_PROB_DISTR,
                "type": str,
                "help": "Analytical probability distribution to be applied over the randomized sampling. Takes strings: "
                + '"Normal", "Poisson", "Exponential", "Beta", "Gamma" [default {}]'.format(
                    str(self.LATIN_HYPERCUBE_PROB_DISTR)
                ),
            },
        }
    ...
    def sample_doepy(
        ...
        fractional_factorial_resolution: int,
        central_composite_center,
        central_composite_face: str,
        central_composite_alpha: str,
        box_behnken_centers: int,
    ):  # Union([dict, str])
        if type(doe_spec) is str:
            ...
            if os.path.isfile(doe_spec_fname):
                ...
                raise Exception(
                    "DOE levels grid file " + str(doe_spec) + " does not exist"
                )
        elif type(doe_spec) is dict:
            doe_spec_dict = doe_spec
        else:
            raise Exception(
                "doe_spec argument in function sample_doepy is "
                + str(type(doe_spec))
                + " ,must be either file path or a dictionary"
            )
        ...
        if doe_algo == self.FULL_FACTORIAL:
            ...
            doe_out_df = build.frac_fact_res(
                doe_spec_dict, fractional_factorial_resolution
            )
            ...
            doe_out_df = build.central_composite(
                doe_spec_dict,
                center_pair,
                central_composite_alpha,
                central_composite_face,
            )
        elif doe_algo == self.LATIN_HYPERCUBE:
            doe_out_df = build.lhs(
                doe_spec_dict,
        ...
        self._doepy_logger.info(
            "DOE table with " + str(doe_out_df.shape[0]) + " entries has been generated"
        )
        doe_out_df.to_csv(
            self.get_doe_results_file_name(report_file_prefix), index=False
        )
        return doe_out_df
