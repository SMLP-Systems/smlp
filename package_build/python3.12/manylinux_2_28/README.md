...
```bash
python3.12 -m pip install --user --force-reinstall --break-system-packages setuptools
python3.12 -m pip install --user meson ninja z3-solver==4.8.12
```
...
- Downloads and compiles **Boost 1.83** (cached in `~/.local/boost_py312`, ~5 min first time)
...
```bash
python3.12 -m pip install dist/smlp-*.whl
```

> **Portability note:** The plain wheel (`linux_x86_64`) embeds RPATH entries pointing to
> `~/.local/boost_py312/lib`, `~/.local/gmp/lib`, and `~/.local/lib/python3.12/site-packages/z3/lib`.
...
```bash
python3.12 -m pip install --user auditwheel patchelf
```
...
```bash
python3.12 -c "import smlp; print('smlp imported OK')"
```
...
| `BOOST_CACHE_DIR` | `~/.local/boost_py312` | Where to cache the compiled Boost |
...
| `Z3_PREFIX` | `~/.local/lib/python3.12/site-packages/z3` | Reuse an existing Z3 install |
...
```bash
python3.12 -m pip uninstall smlp -y
python3.12 -m pip install .
```
...
```bash
rm -rf ~/.local/boost_py312 ~/.local/gmp
python3.12 -m pip install .
```
...
```bash
python3.12 -m pip install --user z3-solver==4.8.12
```
