...
files_to_ignore_from_diff = [
    "Test41_doe_two_levels_doe.csv",
    "Test42_doe_two_levels_doe.csv",
]
    ...
    if "-mode " in switches or "--mode " in switches:
        ...
        if mode == "t":
            if mode_prefix3.startswith("tra"):
                ...
                raise Exception("Unknown mode prefix " + str(mode_prefix3))
            # return 'train'
        elif mode == "p":
            return "prediction"
        elif mode == "f":
...
def main():
    ...
    parser.add_argument(
        "-t",
        "--tests",
        help="Specify tests to run. It can be a comma-separated list of test numbers\
    ...
    parser.add_argument(
        "-m",
        "--modes",
        help="Specify modes (e.g., verify) of tests to run, default is all modes.",
    )
    parser.add_argument(
        "-models",
        "--models",
        help="Specify models (e.g., dt_sklearn) of tests to run, default is all modes.",
    )
    parser.add_argument(
        "-extra",
        "--extra_options",
        help="Specify command line options that will be appended to the command line.",
    )
    ...
    parser.add_argument(
        ...
        "--print_command",
        action="store_true",
        help="print the command to run manually;\
    ...
    parser.add_argument(
        ...
        "--no_all",
        action="store_true",
        help="Answer no to all file replacements/updates\
    ...
    parser.add_argument(
        ...
        "--fail_txt",
        action="store_true",
        help="Don't compare all files if .txt main log\
    ...
    if args.ignore_tests:
        if "," in args.ignore_tests:
            ignored_tests = (
                args.ignore_tests.replace(" ", "").replace("'", "").split(",")
            )
        else:
    ...
    if tests == "all":
        ...
        with open(tests_data, "r") as rFile:
            ...
            for row in csvreader:
                if (
                    row[1].startswith("smlp_toy")
                    or row[1].startswith("mlbt_toy")
                    or row[2].startswith("smlp_toy")
                    or row[2].startswith("mlbt_toy")
                    or (
                        conf_identifier(row[3])
                        and get_conf_name(row[3]).startswith("smlp_toy")
                    )
                    or (not conf_identifier(row[3]) and row[1] == "" and row[2] == "")
        ...
        for e in t_list:
            if ":" in e:  # this option to support tests range, eg: 5:10
                ...
                e_range = e.split(":")
                t_list = t_list + [
                    str(e) for e in list(range(int(e_range[0]), int(e_range[1]) + 1))
                ]
        ...
        while True:
            ...
            if execute_test:
                ...
                if (
                    ...
                    model_name = path.join(models_path, test_data).replace("\\", "/")
    ...
    def smlp_txt_file(fname):
        if (
            "config" in fname
            or "error" in fname
            or "mrmr_features_summary" in fname
            or "_formula" in fname
        ):
            return False
            ...
            if execute_test:
                ...
                for file in new_files:
                    ...
                    if path.exists(master_file):
                        ...
                        exclude_cond = (
                            file_name in files_to_ignore_from_diff
                            or file_name.endswith("_model_term.json")
                        )
                        if (
                            new_file.endswith(".csv")
                        ...
                        if file.endswith("smlp_error.txt"):
                            ...
                            if not args.default:
                                ...
                                if user_input == "1":
                                    ...
                                    if os.path.isdir(new_file):
                                        copytree(
                                            new_file, master_file, dirs_exist_ok=True
                                        )
                                    else:
                                        copyfile(new_file, master_file)
                                elif user_input == "3":
                                    copyfile(new_file, master_file)
                                    copyfile(
                                        new_file, path.join(models_path, file_name)
                                    )
                                elif user_input == "4":
