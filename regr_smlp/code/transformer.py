...
class MultiHeadAttention(nn.Module):
    ...
    def scaled_dot_product_attention(self, Q, K, V, mask=None):
        attn_scores = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(self.d_k) #TODO !!! why negative dims -2 and -1?
        if mask is not None:
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, enc_output, src_mask, tgt_mask):
        ...
        x = self.norm1(x + self.dropout(attn_output))
        attn_output = self.cross_attn(x, enc_output, enc_output, src_mask) # TODO redefining attn_output????
...
class Transformer(nn.Module):
    ...
    def train_transformer_model(self): 
        ...
        for epoch in range(10): # TODO -- 100 or coammand line value
            ...
            loss = loss_func(output.contiguous().view(-1, self.tgt_vocab_size), tgt_data[:, :-1].contiguous().view(-1)) # TODO check view(-1)
            loss.backward()
            optimizer.step()
...
class LanguageModel:  
    def __init__(self):
        ...
        self.block_size = 8 # maximum context length for predictions
    ...
    def read_text(self, path_to_file):
        ...
        data = torch.tensor(self.encode(text), dtype=torch.long); print('data', data.shape, data.dtype, data[ : 1000])
        ...
        for split in ['train', 'val']:
            ...
            for k in range(self.eval_iters):
                ...
                if self.use_blm:
                    ...
                    #logits, loss = BigramLanguageModel(self.vocab_size).forward(pred)
    ...
    def flow(self, path_to_file):
        ...
        print('inputs', xb.shape, 'targets', yb.shape); print(yb)
        
        if self.use_blm:
            model = BigramLanguageModel(self.vocab_size, self.block_size)
        else:
        ...
        for iter in range(self.max_iters):
            ...
            if self.use_blm:
                ...
                #print('predictions', pred.contiguous().view(-1, self.vocab_size))
                loss = loss_func(pred.contiguous().view(-1, self.vocab_size), yb.contiguous().view(-1)); print('loss', loss)
                #logits, loss = BigramLanguageModel(self.vocab_size).forward(pred)
        ...
        print(self.decode(BigramLanguageModel(self.vocab_size, self.block_size).generate(context, max_new_tokens=500)[0].tolist())) 
        print('------------------done------------------')
