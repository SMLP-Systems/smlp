...
def main():
    ...
    wheels = sorted(
        [
            w for w in dist_dir.glob("smlptech-*.whl")
    ...
    if not wheels:
        wheels = sorted(dist_dir.glob("smlptech-*.whl"), key=lambda p: p.stat().st_mtime)

    if not wheels:
        print(f"ERROR: No smlptech wheel found in {dist_dir}/")
        sys.exit(1)
